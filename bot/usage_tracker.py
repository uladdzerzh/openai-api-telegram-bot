...
class UsageTracker:
    ...
    if os.path.isfile(self.user_file):
      ...
      if airtable_api_key and airtable_base_id:
        ...
        if response.status_code == 200:
          ...
          print(
              f"Error adding user {user_name} (ID: {user_id}) to Airtable. Status Code: {response.status_code}"
          )
  ...
  def add_tts_request(self, text_length, tts_model, tts_prices):
    ...
    if str(today) in self.usage["usage_history"]["tts_characters"][tts_model]:
      ...
      self.usage["usage_history"]["tts_characters"][tts_model][str(
          today)] += text_length
    else:
      # create new entry for current date
      self.usage["usage_history"]["tts_characters"][tts_model][str(
  ...
  def initialize_all_time_cost(self,
                               tokens_price=0.002,
    ...
    transcription_cost = round(total_transcription_seconds * minute_price / 60,
                               2)

    total_vision_tokens = sum(
        self.usage['usage_history']['vision_tokens'].values())
    vision_cost = round(total_vision_tokens * vision_token_price / 1000, 2)

    total_characters = [
        sum(tts_model.values()) for tts_model in self.usage['usage_history']
        ['tts_characters'].values()
    ]
