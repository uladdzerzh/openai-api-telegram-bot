...
async def is_user_in_group(update: Update, context: CallbackContext,
                           user_id: int) -> bool:
  """
...
async def edit_message_with_retry(context: ContextTypes.DEFAULT_TYPE,
                                  chat_id: int | None,
                                  message_id: str,
                                  text: str,
                                  markdown: bool = True,
...
def get_remaining_budget(config,
                         usage,
                         update: Update,
                         is_inline=False) -> float:
  """
...
def is_within_budget(config, usage, update: Update, is_inline=False) -> bool:
  ...
  if user_id not in usage:
    usage[user_id] = UsageTracker(user_id, name)
  remaining_budget = get_remaining_budget(config,
                                          usage,
                                          update,
                                          is_inline=is_inline)
...
def add_chat_request_to_usage_tracker(usage, config, user_id, used_tokens):
  ...
  try:
    if int(used_tokens) == 0:
      logging.warning(
          'No tokens used. Not adding chat request to usage tracker.')
      return
...
async def handle_direct_result(config, update: Update, response: any):
  ...
  if kind == 'photo':
    if format == 'url':
      ...
      await update.effective_message.reply_photo(**common_args,
                                                 photo=open(value, 'rb'))
  elif kind == 'gif' or kind == 'file':
    if format == 'url':
      await update.effective_message.reply_document(**common_args,
                                                    document=value)
    if format == 'path':
      await update.effective_message.reply_document(**common_args,
                                                    document=open(value, 'rb'))
  elif kind == 'dice':
...
async def update_airtable_available_budget(user_id, user_name,
                                           available_budget):
  ...
  if records:
    ...
    record_fields = {"Available Budget": float(available_budget)}
    ...
    payload = {"records": [{"id": record_id, "fields": record_fields}]}

    # Perform the PATCH request
    update_response = requests.patch(airtable_url,
                                     headers=headers,
                                     json=payload)

    if update_response.status_code == 200:
      print(
          f"Record for user {user_name} (ID: {user_id}) updated successfully.")
    else:
      print(
