...
from telegram import InputTextMessageContent, BotCommand, SuccessfulPayment
...
class ChatGPTTelegramBot:
  ...
  def __init__(self, config: dict, openai: OpenAIHelper):
    ...
    self.commands = [
        BotCommand(command='buy',
                   description=localized_text('buy_description',
                                              bot_language)),
        BotCommand(command='help',
                   description=localized_text('help_description',
                                              bot_language)),
        BotCommand(command='reset',
                   description=localized_text('reset_description',
                                              bot_language)),
        BotCommand(command='stats',
                   description=localized_text('stats_description',
                                              bot_language)),
        BotCommand(command='resend',
                   description=localized_text('resend_description',
                                              bot_language))
    ]
    ...
    if self.config.get('enable_image_generation', False):
      self.commands.append(
          BotCommand(command='image',
                     description=localized_text('image_description',
                                                bot_language)))

    if self.config.get('enable_tts_generation', False):
      self.commands.append(
          BotCommand(command='tts',
                     description=localized_text('tts_description',
                                                bot_language)))

    self.group_commands = [
        BotCommand(command='chat',
                   description=localized_text('chat_description',
                                              bot_language))
    ] + self.commands
  ...
  async def buy(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
      ...
      payment_options_str = os.getenv("PAYMENT_OPTIONS", "1,10,20")
      payment_options = [
          int(option) for option in payment_options_str.split(",")
      ]
      ...
      for option in payment_options:
        ...
        prices = [LabeledPrice(label='Credits', amount=option * 100)
                  ]  # Amount in cents (e.g., $10.00 is 1000 cents)
  ...
  async def pre_checkout_callback(self, update: Update, context: CallbackContext):
    """
    Обрабатывает предоплатные запросы.
    """
    query: PreCheckoutQuery = update.pre_checkout_query
    
    # Здесь можно выполнить дополнительные проверки, например, сравнить invoice_payload
    # и другие детали платежа, чтобы убедиться в их валидности
    
    try:
        # Если все проверки пройдены, подтверждаем запрос на предоплату
        await query.answer(ok=True)
    except Exception as e:
        # В случае ошибки отправляем отрицательный ответ и логируем ошибку
        await query.answer(ok=False, error_message="Не удалось обработать запрос на предоплату.")
        logging.exception(f"Ошибка при обработке запроса на предоплату: {e}")

  async def on_successful_payment(self, update: Update, context: CallbackContext):
      """
      Обрабатывает успешные платежи.
      """
      successful_payment: SuccessfulPayment = update.message.successful_payment
      
      # Здесь вы можете проверить детали платежа, такие как сумма и валюта
      print(f"Пользователь {update.message.chat.id} оплатил {successful_payment.total_amount / 100} {successful_payment.currency}")
      
      # Обновите бюджет пользователя в Airtable, используя информацию из successful_payment
      user_id = update.message.chat.id
      user_name = update.message.chat.username
      amount_paid = successful_payment.total_amount / 100  # Конвертируем из минимальных единиц валюты в доллары
      await add_airtable_budget(user_id=user_id, user_name=user_name, amount_to_increase=amount_paid)
  ...
  async def stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if remaining_budget < float('inf'):
      text_budget += (f"{localized_text('stats_budget', bot_language)}"
  ...
  async def image(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _generate():
      try:
        ...
        user_id = update.message.from_user.id
        price = self.usage[user_id].add_image_request(
        ...
        await subtract_airtable_budget(update.message.from_user.id,
                                       update.message.from_user.name, price)
      except Exception as e:
        logging.exception(e)
        await update.effective_message.reply_text(
            message_thread_id=get_thread_id(update),
            reply_to_message_id=get_reply_to_message_id(self.config, update),
            text=
            f"{localized_text('image_fail', self.config['bot_language'])}: {str(e)}",
            parse_mode=constants.ParseMode.MARKDOWN)

    await wrap_with_indicator(update, context, _generate,
  ...
  async def tts(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def _generate():
      try:
        ...
        # add image request to users usage tracker
        user_id = update.message.from_user.id
        price = self.usage[user_id].add_tts_request(text_length,
                                                    self.config['tts_model'],
                                                    self.config['tts_prices'])
        # add guest chat request to guest usage tracker
        if str(user_id) not in self.config['allowed_user_ids'].split(
            ',') and 'guests' in self.usage:
          self.usage["guests"].add_tts_request(text_length,
                                               self.config['tts_model'],
                                               self.config['tts_prices'])
        await subtract_airtable_budget(update.message.from_user.id,
                                       update.message.from_user.name, price)
        ...
        await update.effective_message.reply_text(
            ...
            reply_to_message_id=get_reply_to_message_id(self.config, update),
            text=
            f"{localized_text('tts_fail', self.config['bot_language'])}: {str(e)}",
            parse_mode=constants.ParseMode.MARKDOWN)
  ...
  async def transcribe(self, update: Update,
    ...
    """
        ...
        """
    if not self.config[
        'enable_transcription'] or not await self.check_allowed_and_within_budget(
            update, context):
      return
    ...
    async def _execute():
      ...
      try:
        ...
        price = self.usage[user_id].add_transcription_seconds(
        ...
        if self.config[
            'voice_reply_transcript'] and not response_to_transcription:
          ...
          price = self.usage[user_id].add_chat_tokens(
    ...
    await wrap_with_indicator(update, context, _execute,
  ...
  async def vision(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
        ...
        """
    if not self.config[
        'enable_vision'] or not await self.check_allowed_and_within_budget(
            update, context):
      return
    ...
    async def _execute():
        ...
        async for content, tokens in stream_response:
          ...
          if i == 0:
            ...
            try:
              ...
              await edit_message_with_retry(context,
                                            chat_id,
                                            str(sent_message.message_id),
                                            text=content,
            ...
            try:
              ...
              await update.effective_message.reply_text(
                  ...
                  reply_to_message_id=get_reply_to_message_id(
                      self.config, update),
                  text=
                  f"{localized_text('vision_fail', bot_language)}: {str(e)}",
                  parse_mode=constants.ParseMode.MARKDOWN)
        except Exception as e:
          logging.exception(e)
          await update.effective_message.reply_text(
      ...
      price = self.usage[user_id].add_vision_tokens(total_tokens,
                                                    vision_token_price)

      allowed_user_ids = self.config['allowed_user_ids'].split(',')
      if str(user_id) not in allowed_user_ids and 'guests' in self.usage:
        self.usage["guests"].add_vision_tokens(total_tokens,
                                               vision_token_price)
      await subtract_airtable_budget(update.message.from_user.id,
                                     update.message.from_user.name, price)
  ...
  async def prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
      ...
      if self.config['stream']:
        ...
        await wrap_with_indicator(update, context, _reply,
                                  constants.ChatAction.TYPING)

      price = add_chat_request_to_usage_tracker(self.usage, self.config,
      ...
      await update.effective_message.reply_text(
          ...
          reply_to_message_id=get_reply_to_message_id(self.config, update),
          text=
          f"{localized_text('chat_fail', self.config['bot_language'])} {str(e)}",
          parse_mode=constants.ParseMode.MARKDOWN)
  ...
  async def inline_query(self, update: Update,
    ...
    await self.send_inline_query_result(update,
                                        result_id,
                                        message_content=query,
                                        callback_data=callback_data)

  async def send_inline_query_result(self,
                                     update: Update,
                                     result_id,
                                     message_content,
                                     callback_data=""):
    ...
    try:
      ...
      inline_query_result = InlineQueryResultArticle(
          ...
          thumb_url=
          'https://user-images.githubusercontent.com/11541888/223106202-7576ff11-2c8e-408d-94ea'
          '-b02a7a32149a.png',
  ...
  async def handle_callback_inline_query(self, update: Update,
    ...
    try:
      if callback_data.startswith(callback_data_suffix):
        ...
        if query:
          ...
          error_message = (f'{localized_text("error", bot_language)}. '
        ...
        if self.config['stream']:
          ...
          await wrap_with_indicator(update,
                                    context,
      ...
      logging.exception(e)
      localized_answer = localized_text('chat_fail',
  ...
  async def check_allowed_and_within_budget(self,
                                            update: Update,
                                            context: ContextTypes.DEFAULT_TYPE,
  ...
  async def send_disallowed_message(self,
                                    update: Update,
  ...
  async def send_budget_reached_message(self,
                                        update: Update,
                                        _: ContextTypes.DEFAULT_TYPE,
                                        is_inline=False):
    """
  ...
  def run(self):
    ...
    application.add_handler(PreCheckoutQueryHandler(
        self.pre_checkout_callback))
    application.add_handler(PreCheckoutQueryHandler(self.pre_checkout_callback))
    application.add_handler(MessageHandler(filters.SUCCESSFUL_PAYMENT, self.on_successful_payment))
    application.add_handler(
